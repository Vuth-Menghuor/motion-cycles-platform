  ...
  <div class="price-summary">
    ...
    <div class="summary-calculations">
      ...
      <div class="summary-row total">
        <span>You need to pay (Incl. taxes)</span>
        <span>${{ (finalTotal || 0).toFixed(2) }}</span>
      </div>
      <button @click="$emit('proceedToCheckout')" class="checkout-btn">
    ...
    <div class="checkout-features">
      ...
      <div class="feature">30 days Easy Returns</div>
    </div>
  </div>
</template>
...
defineEmits(['update:promoCode', 'proceedToCheckout'])

// Hardcoded correct promo code and discount amount
const CORRECT_PROMO = 'BOOKRIDE50'
const PROMO_DISCOUNT = 1250.0
const SHIPPING_AMOUNT = 5.0

// Computed property to format the delivery date
const deliveryDate = computed(() => {
  const date = new Date()
  date.setDate(date.getDate() + 3) // Add a few days for a realistic "delivery by" date
  const options = { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }
  return date.toLocaleDateString('en-US', options)
})

// Computed property to check if the promo code is valid
const isPromoValid = computed(() => {
  return props.promoCode.trim().toUpperCase() === CORRECT_PROMO
})

// Computed property to dynamically change the icon based on validity
...
// Computed property for the discount amount
const discountAmount = computed(() => {
  return isPromoValid.value ? PROMO_DISCOUNT : 0
})

const totalMRP = computed(() => {
  // Calculates total MRP from cart items using the original price
...
const netPrice = computed(() => {
  // Calculates net price by reducing the discount amount from the MRP
  const calculatedNet = totalMRP.value - discountAmount.value
  return calculatedNet > 0 ? calculatedNet : 0 // Ensure net price doesn't go below zero
})

const finalTotal = computed(() => {
  // Final total is the net price plus shipping
